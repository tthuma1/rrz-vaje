...
def send_pose(chain, robot, pt, gripper, orientation=None):
	...
	ik = chain.inverse_kinematics(pt, target_orientation, orientation, optimizer='scalar')
...
def move_linear(chain, robot, start, end, steps=20, gripper=1.0, orientation=None):
	# linearno interpoliraj od start do end v `steps` korakih
	for t in np.linspace(0, 1, steps):
		pt = (1 - t) * start + t * end
		send_pose(chain, robot, pt, gripper, orientation)
...
def main():
	...
	for i in range(NUM_CUBES):
		...
		print(i)
		if i >= 2: # ne vem zakaj se pri tretji kocki malo pokvari
			place_above[0] += 0.005
			place_above[1] += 0.005
			place_above[2] += 0.005
		...
		# descend to pick
		move_linear(my_chain, robot, pick_above, pick_grasp, gripper=0.7, orientation='all')
		time.sleep(1)

		# close gripper
		send_pose(my_chain, robot, pick_grasp, gripper=0, orientation='all')
		time.sleep(1)
